...
透過本次結合學理分析與網頁實作的觀察，我得出了以下幾點深刻的心得與結論：
    ...
    再來，同為 O(n²)，細節實作仍有高下之分 實驗中令人驚訝的是，氣泡、選擇、插入這三者雖然複雜度相同，但「插入排序」的表現卻遠優於另外兩者。經過思考與對照原理，我發現是因為插入排序在內層迴圈「只要找到正確位置就會提早結束」，而氣泡與選擇排序往往需要跑完完整的比較次數。這提醒了我在撰寫程式碼時，即使無法改變整體架構，加入提早結束（Early Exit）的判斷依然能大幅節省資源。    
