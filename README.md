...
透過本次結合學理分析與網頁實作的觀察，我得出了以下幾點深刻的心得與結論：
    ...
    首先，時間複雜度決定了演算法的極限 在沒有親自跑過數據前，很難體會 O(n²) 與 O(n log n) 的實際差距。實測結果精準地印證了理論：當資料規模一旦擴大，演算法本身的體質（複雜度）才是決定效能的唯一關鍵。硬體的運算速度無法彌補氣泡排序那種缺乏效率的反覆交換機制。    
    其次，同為 O(n²)，細節實作仍有高下之分 實驗中令人驚訝的是，氣泡、選擇、插入這三者雖然複雜度相同，但「插入排序」的表現卻遠優於另外兩者。經過思考與對照原理，我發現是因為插入排序在內層迴圈「只要找到正確位置就會提早結束」，而氣泡與選擇排序往往需要跑完完整的比較次數。這提醒了我在撰寫程式碼時，即使無法改變整體架構，加入提早結束（Early Exit）的判斷依然能大幅節省資源。    
    最後，沒有絕對完美的演算法，只有最適合的選擇 雖然合併排序在本次速度測試中稱王，但回顧理論分析，它需要額外的 O(n) 記憶體空間來進行陣列合併；相對地，堆積排序雖然速度稍慢於合併排序，但它是原地排序（In-place），極度節省記憶體。因此，未來的實務應用上，必須根據「追求極致速度」還是「記憶體資源有限」來做出權衡（Trade-off）。   
   本次實驗藉由視覺化的動畫演示與實際的效能跑分，將原本抽象枯燥的「演算法」與「時間複雜度」轉化為肉眼可見的具體數據。這不僅加深了我對這五大經典排序法的理解，更讓我學會了在未來面對不同規模的工程問題時，該如何有邏輯地挑選最合適的解決方案。
